...
export default function AvailableQuestionsView({
  availableQuestions,
  questionsAddedToSet,
  addToSetCallback,
}: {
  availableQuestions: QuestionModel[] | undefined;
  questionsAddedToSet: QuestionModel[] | undefined;
  ...
  const questionListElement =
    availableQuestions && questionsAddedToSet ? (
      <List
        ...
        renderItem={(item) => (
          <QuestionCardLarge
            question={item}
            addToSetCallback={addToSetCallback}
            isContainedInSet={questionsAddedToSet
              .map((q) => q.id)
              .includes(item.id)}
          />
        )}
    ...
    <div className={styles.availableQuestions}>
      ...
      <div>
        ...
        >
          {availableQuestions &&
            arrayDistinct(
              availableQuestions.map((question) => question.category)
            ).map((category) => (
